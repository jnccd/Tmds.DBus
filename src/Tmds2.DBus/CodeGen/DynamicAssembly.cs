...
[assembly: InternalsVisibleTo(Tmds2.DBus.Connection.DynamicAssemblyName)]
...
namespace Tmds2.DBus.CodeGen
{
    internal class DynamicAssembly
    {
        ...
        {
            byte[] keyBuffer;
            using (Stream keyStream = typeof(DynamicAssembly).GetTypeInfo().Assembly.GetManifestResourceStream("Tmds2.DBus.sign.snk"))
            {
                if (keyStream == null)
                {
                    throw new InvalidOperationException("'Tmds2.DBus.sign.snk' not found in resources");
                }
                keyBuffer = new byte[keyStream.Length];
                keyStream.Read(keyBuffer, 0, keyBuffer.Length);
            }

            var dynamicAssemblyName = "Tmds2.DBus.Emit";
