        ...
        private static void AddInterfaceDescription(Type type, DBusInterfaceAttribute interfaceAttribute, List<InterfaceDescription> interfaces)
        {
            ...
            foreach (var member in type.GetMethods())
            {
                ...
                if (isSignal)
                {
                    ...
                    {
                        throw new NotSupportedException($"Signal {memberName} does not support cancellation. See https://github.com/tmds/Tmds2.DBus/issues/15.");
                    }
                ...
                else
                {
                    ...
                    if (parameters.Length > 0 && parameters[parameters.Length - 1].ParameterType == s_cancellationTokenType)
                    {
                        throw new NotSupportedException($"DBus Method {memberName} does not support cancellation. See https://github.com/tmds/Tmds2.DBus/issues/15.");
                    }
